interface Array<T> {
  ...
  lastObject: T;
  mapBy: (prop: string) => T[];
  filterBy: (prop: string, value: string | number, isIncludes: boolean) => T[];
  findBy: (prop: string, value: string | number, isIncludes: boolean) => T[];
  rejectBy: (prop: string, value: string | number, isIncludes: boolean) => T[];
...
//   const cb = isIncludes ? (a: any) => a[prop]?.includes(value) : (a) => a[prop] === value;
